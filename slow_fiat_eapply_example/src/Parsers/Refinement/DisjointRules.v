...
Global Arguments search_for_condition {_ _} G {_} _ _ _.
...
Proof.
  ...
  left.
  pose proof (terminals_disjoint_search_for _ H_disjoint pit pits) as H'.
...
Global Arguments search_for_not_condition {_ _} G {_ _} _ _ _ _.
...
Proof.
  ...
  inversion Heq; subst it' its'; clear Heq.
  left.
  pose proof (terminals_disjoint_search_for_not _ H_disjoint pit pits) as H'.
  specialize (H1 (ex_intro _ n H')).
  pose proof (is_first_char_such_that_eq_nat_iff H1 H') as H''.
  destruct_head or; destruct_head and; subst;
...
: refine {splits : list nat
         | split_list_is_complete
             ...
             (NonTerminal nt::its)
             splits}
         (ret [find_first_char_such_that (substring offset len str) (fun ch => char_in ch (possible_first_terminals_of_production G its))]).
Proof.
  rewrite refine_disjoint_search_for' by eassumption.
...
: refine {splits : list nat
         | split_list_is_complete
             ...
             (NonTerminal nt::its)
             splits}
         (ret [find_first_char_such_that (substring offset len str) (fun ch => negb (char_in ch (possible_terminals_of G nt)))]).
Proof.
  rewrite refine_disjoint_search_for_not' by eassumption.
  setoid_rewrite refine_find_first_char_such_that.
  simplify with monad laws; reflexivity.
Qed.
...
Ltac pose_disjoint_search_for lem :=
  ...
  let lem' := constr:(@refine_disjoint_search_for_idx HSLM HSL _ G) in
  ...
  | [ |- appcontext[ParserInterface.split_list_is_complete_idx ?G ?str ?offset ?len ?idx] ]
    => pose proof (lem idx) as lem';
       ...
       let x := match type of lem' with
                | context[possible_first_terminals_of_production ?G ?ls]
                  => constr:(possible_first_terminals_of_production G ls)
                end in
