...
Module Import Bounds.
  ...
  Definition of_Z (z : Z) : t := ZToBounds z.

  Definition of_interp t (z : Syntax.interp_base_type t) : interp_base_type t
    := ZToBounds (match t return Syntax.interp_base_type t -> Z with
                  | TZ => fun z => z
  ...
  Definition is_bounded_byb {T} : interp_base_type T -> Syntax.interp_base_type T -> bool
    := fun bound val
       => (((lower bound <=? interpToZ val) && (interpToZ val <=? upper bound))
  ...
  Definition is_bounded_by_bool {T} : interp_flat_type interp_base_type T -> interp_flat_type Syntax.interp_base_type T -> bool
