...
Section refine_rules.
  ...
  Context {G : pregrammar' Ascii.ascii}
          {pchdata : possible_data G}
          (Hvalid : grammar_rvalid G)
          {str : StringLike.String} {n m : nat} {nt : string} {ch : Ascii.ascii} {its : production Ascii.ascii}
	  {pf : let ls := possible_first_terminals_of_production G its in
                if possible_terminals_empty ls
                then False
                else fold_right and True (possible_terminals_map (eq ch) ls)}
  ...
  Section general_table.
    ...
    Proof.
      ...
      {
        let b := match type of pf with
                 | if ?b then False else _ => b
                 end in
        destruct b eqn:terminals.
        {
          simpl in *.
          tauto.
        }
        {
          ...
          pose proof (possible_first_terminals_of_production_is_char pits _ pf) as H_first_char.
          destruct_head or; try omega; [].
          destruct_head ex; destruct_head and; subst.
          assumption.
        }
      }
...
Ltac setoid_rewrite_refine_binop_table_idx args :=
        ...
        match ps with
          ...
          match p with
            ...
            let lem := constr:(lem G) in
            let lem := constr:(lem _) in
            let lem := constr:(fun its' H' ch H0 H1 => lem eq_refl str offset len nt ch its' H0 H1 idx H') in
